...
#  parser.add_argument("--store_path", default="./logs", type=str, help="Where to save the trained models and logs")
  ...
  parser.add_argument("--input_data", default="./libri-light_data/preprocessed/merge_tr_cv_without_pad/train-unlab_600_subset3600utt", type=str, help="Path to the input data directory")
...
#  logging.info('Initial Learning Rate: %f ' % (config.learning_rate))
  ...
  with torch.set_grad_enabled(False):
    i = 0;
    with open(os.path.join(abs_output_data_path_hid_layer, "feats.ark"), "w") as f:
  ...
  ###Replace this sentence with others###
  #      torch.save(feat_to_store, os.path.join(abs_output_data_path_hid_layer, utt_id ))
  ############
        feat_to_store_cpu = feat_to_store.cpu()
        np_feat_to_store = feat_to_store_cpu.numpy()
        f.write(utt_id[:-3] + " [\n") # filename.pt -> filename
        for item_np in np_feat_to_store[:-1,:]:
          f.write(' '.join([str(x_np) for x_np in item_np]) + '\n')
        f.write(' '.join([str(x_np) for x_np in np_feat_to_store[-1,:] ]))
        f.write(" ]\n")
        if i % 100 == 0:
